...
fn easy_ticket(title: String, description: String, status: Status) -> Ticket {
    match Ticket::new(title.clone(), description.clone(), status.clone()){
        ...
        Err(TicketNewError::DescriptionError(_)) => Ticket::new(title, String::from("Description not provided"), status).unwrap()
    }
}
...
impl Ticket {
    pub fn new(
        ...
        if title.is_empty() {
            return Err(TicketNewError::TitleError(String::from("Title cannot be empty")));
        }
        if title.len() > 50 {
            return Err(TicketNewError::TitleError(String::from("Title cannot be longer than 50 bytes")));
        }
        if description.is_empty() {
            return Err(TicketNewError::DescriptionError(String::from("Description cannot be empty")));
        }
        if description.len() > 500 {
            return Err(TicketNewError::DescriptionError(String::from("Description cannot be longer than 500 bytes")));
        }
